...
class TestAuthentication:
    ...
    def test_auth_disabled_by_default(self):
        with patch.dict(
            os.environ, {"ALLOW_UNAUTHENTICATED_ROUTER": "true"}, clear=True
        ):
            importlib.reload(sandbox_router)
            from fastapi.testclient import TestClient
...
class TestProxyRouting:
    ...
    def test_target_url_pod_ip_construction(self, client):
        ...
        with patch.object(
            ...
            built_request = mock_send.call_args
            request_obj = built_request[0][0]
            assert "10.20.30.40:9999/some/path" in str(request_obj.url)
    ...
    def test_target_url_ipv6_pod_ip_construction(self, client):
        ...
        with patch.object(
            ...
            built_request = mock_send.call_args
            request_obj = built_request[0][0]
            assert "[2001:db8::1]:9999/some/path" in str(request_obj.url)
    ...
    def test_request_body_streamed(self, mock_send, client):
        ...
        assert hasattr(sent_request.stream, "__aiter__"), (
...
class TestWebSocketResourceLimits:
    ...
    def test_fractional_connection_limit_falls_back_to_default(
        self, isolated_sandbox_router
    ):
    ...
    def test_backend_websocket_connect_uses_configured_max_message_bytes(self):
        ...
        with patch.dict(
            ...
            client = TestClient(sandbox_router.app)
            with patch(
                "sandbox_router.websockets.connect", side_effect=capture_connect
            ):
                with client.websocket_connect(
                    "/kernels",
                    headers={"X-Sandbox-ID": "my-sandbox"},

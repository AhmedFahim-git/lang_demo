...
def _get_positive_float_env(
    ...
    *,
    ...
    if not math.isfinite(value) or value < 0 or (value == 0 and not allow_zero):
        ...
        print(
            f"WARNING: {name} must be {constraint}, got {value}, "
...
def _get_cluster_domain() -> str:
    ...
    if cluster_domain == "":
        print(
            "WARNING: CLUSTER_DOMAIN must not be an empty string, "
...
def _get_max_keepalive_connections() -> int:
    ...
    try:
        ...
        print(
            f"WARNING: Invalid MAX_KEEPALIVE_CONNECTIONS='{raw}', "
            f"falling back to {DEFAULT_MAX_KEEPALIVE_CONNECTIONS}"
        )
        return DEFAULT_MAX_KEEPALIVE_CONNECTIONS
    if value < 0:
        print(
            f"WARNING: MAX_KEEPALIVE_CONNECTIONS must be >= 0, got {value}, "
            f"falling back to {DEFAULT_MAX_KEEPALIVE_CONNECTIONS}"
        )
        return DEFAULT_MAX_KEEPALIVE_CONNECTIONS
    return value
...
print(
    f"Sandbox router configured with max_keepalive_connections: {max_keepalive_connections}"
)
...
print(f"Sandbox router WebSocket max message size: {websocket_max_message_bytes} bytes")
...
if ROUTER_AUTH_TOKEN:
    ...
    print(
        "WARNING: Running in UNAUTHENTICATED mode because "
...
async def proxy_websocket(websocket: WebSocket, full_path: str):
    ...
    if subprotocol_header:
        subprotocols = [
            item.strip() for item in subprotocol_header.split(",") if item.strip()
        ]
    ...
    try:
        ...
        await websocket.close(
            code=1011, reason="An internal error occurred in the proxy."
        )
    finally:
